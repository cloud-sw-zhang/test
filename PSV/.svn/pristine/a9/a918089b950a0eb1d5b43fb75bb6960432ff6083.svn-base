...
def check_DUTnocrash(url):
    ...
    if ProjectName == 'CM500V-1STAUS':
        try:
            AclStatus = driver.find_element_by_name("refresh").get_attribute('value')
            driver.quit()
        except:
            time.sleep(1)
        if AclStatus == 'Cancel':
...
def aploaddefault():
    ...
    if console_type != 'cable':
        ...
        print str
        time.sleep(3)
        if str.find('done') != -1 or str.find('finished') != -1 or str.find('RST button') != -1 or str.find('commit by') != -1:
